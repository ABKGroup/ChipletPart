...
#Number of wires: bandwidth_between_of_blocks / bandwidth_of_io * wire_count_of_io
#Reach of the io 
#Actual size of the IO:  size_of_io * bandwidth_between_of_blocks / bandwidth_of_io
#Block IDs

#num_wires reach total_area id_0 id_1
...
def convertXMLtoHypergraph(netlist_file:str, block_def_file:str, io_def_file:str, output_hgraph: str) -> None:
    ...
    for io in root.findall('io'):
        ...
        io_type_to_bw[io.attrib['type']] = float(io.attrib['bandwidth'])
        io_type_to_wire_count[io.attrib['type']] = int(io.attrib['wire_count'])
        io_type_to_area[io.attrib['type']] = float(io.attrib['tx_area'])
    ...
    for net in root.findall('net'):
        ...
        net_bw = float(net.attrib['bandwidth'])
        io_bw = io_type_to_bw[type]
        io_count = math.ceil(net_bw/io_bw)
        io_wire_count = io_type_to_wire_count[type]
        io_area = io_count * io_type_to_area[type]
        net_wt = io_wire_count * io_count
        net_array.append([block0_index, block1_index])
        net_wt_array.append(net_wt)
        net_area_array.append(io_area)
    ...
    for line in f:
        vertex, area, power, tech, is_memory = line.split()
    ...
    edge_idx = 0
    for edge_idx in range(len(net_array)):
        f.write(str(net_wt_array[edge_idx]) + ' ' + str(reach_array[edge_idx]) + ' ' + str(net_area_array[edge_idx]) + " ")
        for vertex in net_array[edge_idx]:
